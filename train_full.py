...
if DATASET_NAME == 'MIMIC':
    ...
    EPOCHS = 50 # Start overfitting after 20 epochs
    BATCH_SIZE = 32 if PHASE == 'TRAIN' else 64 # Fit 4 GPUs
    MILESTONES = [25] # Reduce LR by 10 after reaching milestone epochs
...
#         SOURCES = ['image','caption','label']
#         TARGETS = ['caption','label',]
#         KW_SRC = ['image','caption','label']
        KW_TGT = None
        KW_OUT = None
    ...
    if DATASET_NAME == 'MIMIC':
        ...
        dataset = MIMIC('Vocabulary/', INPUT_SIZE, view_pos=['AP','PA','LATERAL'], max_views=MAX_VIEWS, sources=SOURCES, targets=TARGETS)
    ...
    if BACKBONE_NAME == 'ResNeSt50':
        ...
        #backbone = torch.hub.load('pytorch/vision', 'densenet121', pretrained=True)
        FC_FEATURES = 1024
    ...
    if MODEL_NAME == 'ClsGen':
        ...
        if not RELOAD:
            checkpoint_path_from = 'checkpoints/NLMCXR_ClsGen_DenseNet121_MaxView2_NumLabel114_History.pt'
    ...
    if PHASE == 'TRAIN':
        ...
        loss_df.to_csv(f'{MODEL_NAME}__train_loss.csv', index=False)
        ...
        out_file_ref = open('outputs/{}_{}_{}_{}_Ref.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT), 'w')
        out_file_hyp = open('outputs/{}_{}_{}_{}_Hyp.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT), 'w')
        out_file_lbl = open('outputs/{}_{}_{}_{}_Lbl.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT), 'w')
